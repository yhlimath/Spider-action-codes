...
def sweep():
    ...
    print(f"Starting sweep: n={n:.2f}, {len(x_vals)} x values, {len(y_vals)} y values.")

    for j, y_val in enumerate(y_vals):
        for i, x_val in enumerate(x_vals):
            lam_dict = {}
            for L in L_list:
                solver = KuperbergArnoldiSolver(L, 0, 0, 'T(x,y)', order, n, x_value=x_val, y_value=y_val)
                if solver.dim > 0:
                    if solver.dim <= 5:
    ...
    plt.title(f"Phase Diagram for T(x,y) | n={n:.2f}")
    plt.xlabel("Boltzmann weight x")
    plt.ylabel("Boltzmann weight y")

    # Format range string safely
    filename = f"phase_diagram_C_n{n:.2f}_x{args.x_start}-{args.x_stop}_y{args.y_start}-{args.y_stop}.png"
    plt.savefig(os.path.join(out_dir, filename), dpi=150)
    plt.close()

    with open(os.path.join(out_dir, f"phase_diagram_data_n{n:.2f}.json"), 'w') as f:
