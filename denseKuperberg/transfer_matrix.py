...
from denseKuperberg.algebra import action_E_i, action_H_i, action_T_xy_i
...
def apply_T_i(coeff, path, i, type_str, n_value, x_value=None, y_value=None):
    ...
    if type_str == 'T(x,y)':
        res = apply_action(base_state, lambda c, p, idx: action_T_xy_i(c, p, idx, x_value, y_value, n_value), i, n_value)
        add_to_result(res)
        return result
...
def apply_layer(state_dict, indices, type_str, n_value, x_value=None, y_value=None):
    """
    ...
    for i in indices:
        ...
        for p_tup, c in current_state.items():
            res = apply_T_i(c, list(p_tup), i, type_str, n_value, x_value, y_value)
...
def build_transfer_matrix(L, x, y, type_str, order_str, n_value, x_value=None, y_value=None):
    """
    ...
    for idx, p in enumerate(paths):
        ...
        for layer_indices in layers:
            state = apply_layer(state, layer_indices, type_str, n_value, x_value, y_value)
